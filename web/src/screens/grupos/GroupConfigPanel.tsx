...
// Sheets de "adicionar membro" e ConfirmDialog renderizam como IRMÃOS do
// BottomSheet principal (não aninhados) — evita briga de z-index/scroll-lock.
...
export function GroupConfigPanel({ open, group, onClose }: Props) {
  const { role } = useAuth();
  ...
  return (
    <>
      ...
      <BottomSheet open={open && addOpen} onClose={() => setAddOpen(false)} title={`Adicionar membro — ${group.name}`}>
