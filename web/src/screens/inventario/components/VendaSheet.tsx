...
// Sheet para registrar venda de produto da lojinha.
...
import { registrarVenda } from '../../../lib/lareport-mutations';
import type { VendaInput } from '../../../lib/lareport-mutations';
...
export function VendaSheet({ open, onClose, unidadeId, onSuccess }: Props) {
  const [termo, setTermo] = useState('');
  const [produtoId, setProdutoId] = useState<number | null>(null);
  const [produtoNome, setProdutoNome] = useState('');
  const [quantidade, setQuantidade] = useState(1);
  const [formaPgto, setFormaPgto] = useState<VendaInput['forma_pagamento']>('pix');
  const [tipoCliente, setTipoCliente] = useState<VendaInput['tipo_cliente']>('avulso');
  ...
  async function submit() {
    ...
    try {
      ...
      showToast({
        ...
        msg: `Saldo pós-venda: ${result.saldo_apos} un.`,
      });
      onSuccess?.();
      onClose();
    } catch (e: any) {
      ...
      <div className="p-md space-y-md">
                  ...
                  {resultados.map(p => (
                    ...
                    >
                      ...
                      <div className="text-body-sm text-fg-muted">
                        SKU: {p.sku} · Estoque: {p.estoque} · R${p.preco.toFixed(2)}
                      </div>
                    </button>
                  ))}
                </div>
        ...
        <Field label="Forma de pagamento">
          <CustomSelect
            ...
            onChange={v => setFormaPgto(v as VendaInput['forma_pagamento'])}
          />
        </Field>
        ...
        <Field label="Tipo de cliente">
          <CustomSelect
            value={tipoCliente ?? 'avulso'}
            options={TIPO_CLIENTE_OPTIONS}
            onChange={v => setTipoCliente(v as VendaInput['tipo_cliente'])}
          />
        </Field>

        {/* Observações */}
        <Field label="Observações" sub="Opcional">
          <input
            type="text"
            value={obs}
            onChange={e => setObs(e.target.value)}
            placeholder="Ex: Aluno João, desconto professor…"
            className="w-full bg-bg-surface border border-border rounded-md p-2 text-fg focus:outline-none focus:border-tom"
          />
        </Field>
