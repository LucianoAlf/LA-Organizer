...
| 09/09 | Fefê | "atualizo a existente?" — o TOM ofereceu EDITAR uma tarefa já criada | feature (`<<TASK_UPDATE>> action:"update"` é recusado como `schema_invalid`: `update` não está em `VALID_TASK_ACTIONS` e não existe handler nenhum — o prompt oferece a capacidade, o executor não tem) | 1 | **FEITO 10/09 (a3ef81da)** — o Alf decidiu implementar: `update` edita nome, acrescenta detalhe e move pra grupo. KI TASK-UPDATE-NAO-EXISTIA.
| 12/09 | Anne Susan | "Tom não existe essa prova de clínica psicanalítica. Pode tirar da lista de lembretes" | feature (apagar memória pelo chat: `persistMemoryRows` em `engine.js:6883` só faz insert/update e **ignora `r.action` por completo** — não existe caminho de delete. O LLM emitiu `<<MEMORY_SAVE>>{"action":"delete","fact":"..."}` e levou `schema_invalid`; `fact` também não está em `CHAVES_CONTEUDO` de `services/memory-fields.js`, mas corrigir só o alias daria uma memória DUPLICADA em vez de um delete) | 1 |
