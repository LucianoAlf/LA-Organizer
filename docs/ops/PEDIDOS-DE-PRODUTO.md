...
| 09/09 | Fefê | "atualizo a existente?" — o TOM ofereceu EDITAR uma tarefa já criada | feature (`<<TASK_UPDATE>> action:"update"` é recusado como `schema_invalid`: `update` não está em `VALID_TASK_ACTIONS` e não existe handler nenhum — o prompt oferece a capacidade, o executor não tem) | 1 |
