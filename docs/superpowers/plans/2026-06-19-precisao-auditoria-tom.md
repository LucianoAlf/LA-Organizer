...
3. **Fix `spawn E2BIG` (achado na validação E2E real):** o ledger tinha **211 known-issues corrigidos em 45d = 252 KB**; mandar todos num prompt estourava o limite de argumento do Claude CLI. Correção em `finding-triage.js`/`finding-triage-prompt.js`: known-issues candidatos = só os `KI_MAX=40` mais recentes (fix antigo não casa com finding ativo), campos curtos (sem `fix_resumo`, `causa_raiz`≤120, `evidence`≤250), e findings processados em **lotes de `BATCH_FINDINGS=20`**. Mocks unit não pegavam — só a execução E2E na VPS revelou.
